...
class GameOfLife:
    def __init__(self, root, rows=100, cols=100):
        ...
        self.glider_input = tk.Entry(root, width=5)
    ...
    def next_generation(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if self.grid[i][j] == 1:
                    new_grid[i][j] = 1 if neighbors in [3, 4] else 0
                else:
                    new_grid[i][j] = 1 if neighbors == 4 else 0
        self.grid = new_grid
...
def main():
    root = tk.Tk()
    root.title("Game of Life - Modified Rules")
    app = GameOfLife(root)
    root.mainloop()
